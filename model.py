...
#                    b = open("./players/gridsize.txt","a")
#                    print("grid size is ",self.GRID_SIZE,file=b)
                    ...
                    tg = TileGrab()
                    tg.number = num_avail #how many same type tile in the factory
                    tg.tile_type = tile
                    tg.pattern_line_dest = i
                    tg.num_to_pattern_line = min(num_avail, slots_free) #pick the minimum from the same type or the space in your pattern line
                    tg.num_to_floor_line = tg.number - tg.num_to_pattern_line #throw whatever is left onto the floor line(junk line)
